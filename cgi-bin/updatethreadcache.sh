...
for thread in $(cd .. && ls -td [0-9]* && cd cgi-bin 2>/dev/null)
do
	if [ -e ../$thread/archive ]
	then
		continue
	...
	do
		...
		tripcode=$(grep -E "^${no} .*$" ../cgi-bin/tripcodes | head -n 1 | sed -r 's/^[0-9]+ (.*)$/\1/')
		postid=$(grep -E "^${no} .*$" ../cgi-bin/postids | head -n 1 | sed -r 's/^[0-9]+ (.*)$/\1/')
